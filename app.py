...
def load_from_spreadsheet():
    ...
    for art in raw_articles:
        ...
        articles.append({
            ...
            "ジャンル": genre,
            "分類": category,
            "メインKW": art.get("メインKW", art.get("スラッグ", slug)),
            "タイトル": title,
            "スラッグ": slug,
            "URL": url,
            "公開日": pub_display,
...
with tab1:
    total = len(filtered)
    alert_count = len(filtered[filtered["PV比"] < 80])
    pos_drop_count = len(filtered[filtered["順位変動"] >= 5])  # 順位5以上悪化
    overdue = len(filtered[filtered["ステータス"] == "未対応"])
    completed = len(filtered[filtered["ステータス"] == "完了"])
    ...
    with c1:
        st.markdown(f"""<div class="kpi-card">
            ...
            <div class="sub">{len(filtered[filtered['分類']=='ノウハウ'])} ノウハウ / {len(filtered[filtered['分類']=='CV'])} CV</div>
